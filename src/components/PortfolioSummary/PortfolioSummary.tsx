...
import { getNextMonthStart, endOfMonthISO, parseDate } from '../../utils/date';
import { getMonthDays } from '../../utils/monthDays';
import PortfolioChart from '../PortfolioChart';
import { ChartYearRange } from '../../enums/ChartYearRange';
import { currentYear, getRangeEndYear } from '../../utils/chartRange';
  ...
  return (
    <section className="portfolio-section" aria-label={t('portfolio.title')}>
      ...
      <PortfolioChart
        ...
        onMonthSelect={(key) => setSelectedMonthKey((prev) => prev === key ? null : key)}
      ...
      {selectedMonthKey ? (
        <PortfolioMonthDetail
          ...
          showZeroInterest={showZeroInterest}
          onShowZeroInterestChange={setShowZeroInterest}
          onToggle={onToggle}
        />
      ) : (
        <p className="portfolio-click-hint">{t('portfolio.chartClickHint')}</p>
      )}
    </section>
  );
}

interface PortfolioMonthDetailProps {
  items: BankAccount[];
  ...
  showZeroInterest: boolean;
  onShowZeroInterestChange: (v: boolean) => void;
  onToggle: (id: string) => void;
}

function PortfolioMonthDetail({
  items,
  selectedMonthKey,
  viewMode,
  hasMixedCurrencies,
  ...
  showZeroInterest,
  onShowZeroInterestChange,
  onToggle,
}: PortfolioMonthDetailProps) {
  const { t } = useTranslation();
    ...
    <>
      ...
      <div className="portfolio-items">
        {visible.map((r) => {
          ...
          const cur = (r.currency ?? globalCurrency) as Currency;
          ...
          return (
            <div key={r.id} className={`portfolio-entry${status === 'expired' ? ' portfolio-item--expired' : ''}${status === 'upcoming' ? ' portfolio-item--upcoming' : ''}`}>
              <div
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onExpandToggle(r.id); } }}
        ...
        <div className="form-checkbox portfolio-inactive-toggle">
          <input type="checkbox" id="showZeroInterest" checked={showZeroInterest} onChange={(e) => onShowZeroInterestChange(e.target.checked)} />
          <label htmlFor="showZeroInterest">{t('portfolio.showZeroInterest')}</label>
        </div>
        <div className="form-checkbox portfolio-inactive-toggle">
          <input type="checkbox" id="showInactive" checked={showInactive} onChange={(e) => onShowInactiveChange(e.target.checked)} />
